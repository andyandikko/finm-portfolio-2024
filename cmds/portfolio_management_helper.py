...
def calc_summary_statistics(
    ...
    This function supports flexible configurations for analyzing both total and excess returns, handling risk-free rate adjustments, and calculating statistics over specified timeframes. Users can also customize the output by selecting specific columns or indexes to keep or drop.

    Parameters:
    ----------
    returns : Union[pd.DataFrame, List]
        Time series of returns. Each column represents an asset, and each row represents a time period (e.g., daily, monthly). 
        Can be a DataFrame or a list of DataFrames/Series to merge.
        
    annual_factor : int, optional (default=None)
        Factor used for annualizing returns and volatility. Common values are:
        - 12 for monthly returns
        - 252 for daily returns
        If not specified, assumes monthly returns with an annual factor of 12.
        
    provided_excess_returns : bool, optional (default=None)
        Whether the input returns are already excess returns (returns above the risk-free rate). 
        If set to False, the function will subtract the risk-free rate provided via `rf`.
        If None, assumes excess returns are provided.

    rf : Union[pd.Series, pd.DataFrame], optional (default=None)
        Risk-free rate data, used to adjust total returns to excess returns. 
        Must have the same index as `returns` if specified. Required if `provided_excess_returns` is False.

    var_quantile : Union[float, List], optional (default=0.05)
        Quantile(s) for calculating Value at Risk (VaR) and Conditional VaR (CVaR). 
        Can be a single float (e.g., 0.05 for 5% quantile) or a list of quantiles for multiple VaR calculations.

    timeframes : dict, optional (default=None)
        Defines specific periods for calculating statistics, allowing for comparison across different time ranges.
        Each key should be a descriptive label (e.g., "YTD", "Q1 2024"), and each value should be a list or tuple with 
        start and end dates `[start_date, end_date]`. Use None for open-ended periods (e.g., ["2024-01-01", None] for data from January 1, 2024, onwards).
        Example:
        ```
        timeframes = {
    ...
    return_tangency_weights : bool, optional (default=True)
        If True, includes tangency portfolio weights in the output, calculated based on the returns provided.

    correlations : Union[bool, List], optional (default=True)
        If True, includes the correlation matrix of returns. Can also specify a list of columns to limit the correlations to specific assets.

    keep_columns : Union[list, str], optional (default=None)
        Specify columns to keep in the resulting DataFrame. Useful for narrowing down the output to relevant metrics.

    drop_columns : Union[list, str], optional (default=None)
        Specify columns to drop from the resulting DataFrame. Applied before or after `keep_columns` based on `drop_before_keep`.

    keep_indexes : Union[list, str], optional (default=None)
        Specify indexes (rows) to keep in the resulting DataFrame.

    drop_indexes : Union[list, str], optional (default=None)
        Specify indexes (rows) to drop from the resulting DataFrame. Applied before or after `keep_indexes` based on `drop_before_keep`.

    drop_before_keep : bool, optional (default=False)
    ...
    -------
    pd.DataFrame
        A DataFrame containing summary statistics of the returns, including:
        - Mean and Annualized Mean
        ...
        - Maximum Drawdown, Peak, Bottom, Recovery Date, and Drawdown Duration
        - Correlation Matrix (if specified)
        - Tangency Portfolio Weights (if `return_tangency_weights` is True)

    Raises:
    ------
    Exception
        - If the `rf` index does not match the `returns` index when `provided_excess_returns` is False.
        - If no returns are found within a specified timeframe.
        - If columns specified for correlation are not present in `returns`.
